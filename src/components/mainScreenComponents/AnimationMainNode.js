...
function AnimationMainNode({ fallAsleepTime, navigation, addAlarm }) {
  ...
  const hoursText = useDerivedValue(() => {
    ...
    let lap = Math.ceil((hoursAnimation.value + lapM * ONE_STEP_VALUE) / (ONE_STEP_VALUE * HOURS));
    let step = Math.ceil((hoursAnimation.value + lapM * ONE_STEP_VALUE) / ONE_STEP_VALUE);
  ...
  const nextHoursText = useDerivedValue(() => {
    let lapM = Math.ceil(minutesAnimation.value / (ONE_STEP_VALUE * MINUTES));
    let step = Math.ceil(((hoursAnimation.value + lapM * ONE_STEP_VALUE) + ONE_STEP_VALUE) / ONE_STEP_VALUE);
    let lap = Math.ceil(((hoursAnimation.value + lapM * ONE_STEP_VALUE) + ONE_STEP_VALUE) / (ONE_STEP_VALUE * HOURS));
  ...
  const nextMinutesText = useDerivedValue(() => {
    let step = Math.ceil((minutesAnimation.value + ONE_STEP_VALUE) / ONE_STEP_VALUE);
    let lap = Math.ceil((minutesAnimation.value + ONE_STEP_VALUE) / (ONE_STEP_VALUE * MINUTES));
  ...
  const rotateValue = useDerivedValue(() => {
    ...
    let lapH = Math.ceil((hoursAnimation.value + lapM * ONE_STEP_VALUE) / (ONE_STEP_VALUE * HOURS));
    let stepH = Math.ceil((hoursAnimation.value + lapM * ONE_STEP_VALUE) / ONE_STEP_VALUE);
  ...
  const recomendText5Phase = useDerivedValue(() => {
    let lapM = Math.ceil((minutesAnimation.value + (fallAsleepTime + 30) * ONE_STEP_VALUE) / (ONE_STEP_VALUE * MINUTES));
    let stepM = Math.ceil((minutesAnimation.value + (fallAsleepTime + 30) * ONE_STEP_VALUE) / ONE_STEP_VALUE);
    stepM = Math.abs(stepM - lapM * MINUTES);

    let lapH = Math.ceil((hoursAnimation.value + (lapM + 7) * ONE_STEP_VALUE) / (ONE_STEP_VALUE * HOURS));
    let stepH = Math.ceil((hoursAnimation.value + (lapM + 7) * ONE_STEP_VALUE) / ONE_STEP_VALUE);
    stepH = Math.abs(stepH - lapH * HOURS);

    return `${stepH < 10 ? `0${stepH}` : String(stepH)}:${stepM < 10 ? `0${stepM}` : String(stepM)}`
  });

  const recomendText6Phase = useDerivedValue(() => {
    let lapM = Math.ceil((minutesAnimation.value + fallAsleepTime * ONE_STEP_VALUE) / (ONE_STEP_VALUE * MINUTES));
    let stepM = Math.ceil((minutesAnimation.value + fallAsleepTime * ONE_STEP_VALUE) / ONE_STEP_VALUE);
    stepM = Math.abs(stepM - lapM * MINUTES);

    let lapH = Math.ceil((hoursAnimation.value + (lapM + 9) * ONE_STEP_VALUE) / (ONE_STEP_VALUE * HOURS));
    let stepH = Math.ceil((hoursAnimation.value + (lapM + 9) * ONE_STEP_VALUE) / ONE_STEP_VALUE);
    stepH = Math.abs(stepH - lapH * HOURS);
  ...
  const recomendText4Phase = useDerivedValue(() => {
    ...
    let lapH = Math.ceil((hoursAnimation.value + (lapM + 6) * ONE_STEP_VALUE) / (ONE_STEP_VALUE * HOURS));
    let stepH = Math.ceil((hoursAnimation.value + (lapM + 6) * ONE_STEP_VALUE) / ONE_STEP_VALUE);
    stepH = Math.abs(stepH - lapH * HOURS);
